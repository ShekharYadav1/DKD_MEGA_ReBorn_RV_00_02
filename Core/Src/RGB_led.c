...
void set_Chnl_PWM(uint8_t red, uint8_t gree, uint8_t blu)
{
	...
	if (red != 0)
	{
		__HAL_TIM_SET_COMPARE(rgb_var.tim_handler, RED_CH_PB0, (red * 4) - 1);
	}
	if (gree != 0)
	{
		__HAL_TIM_SET_COMPARE(rgb_var.tim_handler, GREEN_CH_PA7, (gree * 4) - 1);
	}
	if (blu != 0)
	{
		__HAL_TIM_SET_COMPARE(rgb_var.tim_handler, BLUE_CH_PA6, (blu * 4) - 1);
	}
}
